...
│   ├── deploy-cluster.sh    # Full zero-touch deployment (phases 0-11)
│   ├── destroy-cluster.sh   # Cluster teardown
│   ├── upgrade-cluster.sh   # Cluster upgrade
│   ├── setup-keycloak.sh    # Keycloak OIDC realm + clients
│   ├── setup-gitlab.sh      # GitLab deployment
│   ├── setup-cicd.sh        # ArgoCD + Rollouts CI/CD integration
│   ├── setup-kasm-oidc.sh   # Kasm OIDC integration
│   ├── setup-kubectl-oidc.sh # kubectl OIDC setup
│   ├── prepare-airgapped.sh # Prepare artifacts for airgapped deployment
│   ├── lib.sh               # Shared functions (wait helpers, vault, helm, etc.)
│   └── .env                 # Generated credentials (gitignored)
├── operators/               # Custom Kubebuilder operators (Go 1.25.7)
...
└── docs/                    # Architecture and engineering references
    ...
    ├── airgapped-mode.md    # Airgapped deployment mode (implemented)
...
5. **Tools:** `terraform >= 1.5.0`, `kubectl`, `jq`, `helm`, `openssl`, `curl`, `htpasswd`
...
**Phase overview:** 0: Terraform, 1: Foundation (cert-manager, CNPG, Redis Operator, Node Labeler, Cluster Autoscaler, MariaDB Operator if LibreNMS enabled), 2: Vault + PKI, 3: Monitoring, 4: Harbor, 5: ArgoCD + Rollouts, 6: Keycloak, 7: Remaining Services (Mattermost, Kasm, Uptime Kuma, LibreNMS), 8: DNS Records, 9: Validation + RBAC, 10: Keycloak OIDC + oauth2-proxy ForwardAuth, 11: GitLab.
...
The Traefik Dashboard now uses Gateway API with a ClusterIP Service (`traefik-api` on port 8080).
