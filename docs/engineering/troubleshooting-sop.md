...
kubectl get secret -n argo-rollouts rollouts-basic-auth

# Check TLS
kubectl get secret -n argo-rollouts rollouts-<DOMAIN_DASHED>-tls
```

**Root Cause**: Basic-auth secret missing or incorrect, TLS certificate not issued, or Gateway/HTTPRoute misconfigured.

**Resolution**:
```bash
# 1. Verify basic-auth credentials match .env
kubectl get secret -n argo-rollouts rollouts-basic-auth -o jsonpath='{.data.users}' | base64 -d
...
**Prevention**: Basic-auth credentials are auto-generated by `generate_or_load_env` in `lib.sh`.
...
Used for Grafana, Prometheus, and Argo Rollouts dashboard.
...
# Update secret in kube-system (Traefik middleware)
kubectl patch secret -n kube-system basic-auth \
  -p "{\"data\":{\"users\":\"$(htpasswd -bc /dev/stdin admin $NEW_PASS | base64 -w0)\"}" \
  --type merge

# Middleware will auto-reload from secret
# Next login attempt will use new password
```
